...
#include "VoxelPointBlueprintLibrary.generated.h"

UCLASS()
class VOXELGRAPHCORE_API UVoxelPointBlueprintLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	template<typename T, typename = std::enable_if_t<!TIsTObjectPtr<T>::Value && !std::is_pointer_v<T>>>
	...
	{
		FString Error;
		ensureMsgf(UVoxelPointBlueprintLibrary::SetPointAttribute(
			Handle,
			Name,
