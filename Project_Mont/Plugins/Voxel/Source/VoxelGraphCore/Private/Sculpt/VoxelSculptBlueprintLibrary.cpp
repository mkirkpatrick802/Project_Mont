...
void UVoxelSculptBlueprintLibrary::ApplySculpt(
	...
	AVoxelActor* SculptActor)
{
	...
	const int32 VoxelSize = SculptStorageParameter->VoxelSize;
	...
	const TSharedPtr<FVoxelExecNodeRuntime_EditSculptSurface> NodeRuntime = EditSculptSurfaceParameter->WeakRuntime.Pin();
	if (!NodeRuntime)
	{
	...
	const TOptional<FVoxelBounds> OptionalLocalBounds = FVoxelTaskHelpers::TryRunSynchronously(
		NodeRuntime->GetTerminalGraphInstance(),
		[&]
		{
			const FVoxelQuery Query = FVoxelQuery::Make(
				FVoxelTerminalGraphInstance::MakeDummy(TargetActor->GetRuntime()->GetRuntimeInfoRef().AsShared()),
				MakeVoxelShared<FVoxelQueryParameters>(),
				FVoxelDependencyTracker::Create("VoxelSculpt"))
				.MakeNewQuery(NodeRuntime->GetTerminalGraphInstance());
	...
	const TOptional<FVoxelFloatBuffer> SurfaceDistances = FVoxelTaskHelpers::TryRunSynchronously(
		NodeRuntime->GetTerminalGraphInstance(),
		[&]
		{
			const FVoxelQuery Query = FVoxelQuery::Make(
