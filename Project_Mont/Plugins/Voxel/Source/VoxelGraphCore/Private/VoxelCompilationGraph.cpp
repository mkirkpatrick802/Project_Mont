...
DEFINE_VOXEL_MEMORY_STAT(STAT_VoxelCompilationGraph);
...
TSharedRef<FVoxelMessageToken> FPin::CreateMessageToken() const
{
	FString PinName = Name.ToString();
#if WITH_EDITOR
	if (Node.Type == ENodeType::Struct)
	{
		if (const TSharedPtr<const FVoxelPin> VoxelPin = Node.GetVoxelNode().FindPin(Name))
		{
	...
	Result->AddToken(Node.CreateMessageToken());
	Result->AddText("." + PinName);
	return Result;
}
...
void FNode::AddWarning(const FString& Warning)
{
...
void FNode::ApplyErrors() const
{
	for (const FString& Warning : Warnings)
	{
		VOXEL_MESSAGE(Warning, "{0}: {1}", this, Warning);
	}
...
TSharedRef<FVoxelMessageToken> FNode::CreateMessageToken() const
{
	return NodeRef.CreateMessageToken();
}
...
{
	return NameToPin.FindRef(Name);
}

FPin* FNode::FindInput(const FName Name)
{
	if (FPin* Pin = NameToPin.FindRef(Name))
	{
...
{
	FPin& Pin = *NameToPin.FindChecked(Name);
...
FPin& FNode::NewInputPin(const FName Name, const FVoxelPinType& PinType, const FVoxelPinValue& DefaultValue)
{
	...
	NameToPin.Add_CheckNew(Name, &Pin);
...
void FNode::RemovePin(FPin& Pin)
{
	...
	ensure(NameToPin[Pin.Name] == &Pin);
	NameToPin[Pin.Name] = nullptr;
...
void FGraph::CloneInto(FGraph& Graph) const
{
	VOXEL_FUNCTION_COUNTER();
	ensure(Graph.Nodes.Num() == 0);

	TVoxelMap<const FNode*, FNode*> OldToNewNodes;
	OldToNewNodes.Reserve(Nodes.Num());

	TVoxelMap<const FPin*, FPin*> OldToNewPins;
	{
...
bool FGraph::Identical(const FGraph& Other, FString* OutDiff) const
{
	...
	using FNodeArray = TVoxelInlineArray<const FNode*, 1>;
	TVoxelMap<uint32, FNodeArray> HashToNodesA;
	TVoxelMap<uint32, FNodeArray> HashToNodesB;
	...
	TVoxelMap<const FNode*, const FNode*> NodeAToNodeB;
