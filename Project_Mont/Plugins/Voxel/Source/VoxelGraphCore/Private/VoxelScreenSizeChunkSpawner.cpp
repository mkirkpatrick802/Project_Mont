...
void FVoxelScreenSizeChunkSpawner::Initialize(const FVoxelExecNodeRuntime_OutputMarchingCubeMesh& NodeRuntime)
{
	...
	if (!ChunkScreenSizeValueFactory.IsValid())
	{
		...
		ensure(!TerminalGraphInstance);
		ensure(!QueryParameters);
		return;
	}

	ensure(!ChunkScreenSizeValue.IsValid());
	ChunkScreenSizeValue = ChunkScreenSizeValueFactory.Compute(TerminalGraphInstance.ToSharedRef(), QueryParameters.ToSharedRef());
...
void FVoxelScreenSizeChunkSpawner::Tick(const FVoxelExecNodeRuntime_OutputMarchingCubeMesh& NodeRuntime)
{
	VOXEL_FUNCTION_COUNTER();
	...
	if (!FVoxelUtilities::GetCameraView(NodeRuntime.GetWorld_GameThread(), ViewOrigin))
	{
		return;
	}

	ViewOrigin = NodeRuntime.GetLocalToWorld().Get_NoDependency().InverseTransformPosition(ViewOrigin);
...
void FVoxelScreenSizeChunkSpawner::UpdateTree(const FVector& ViewOrigin)
{
	...
	const int64 SizeInChunks = FMath::Max<int64>(FMath::CeilToInt64(WorldSize / double(GetVoxelSize() * ChunkSize)), 2);
	...
	AsyncBackgroundTask(MakeWeakPtrLambda(this, [this, ViewOrigin, OctreeDepth, OldTree = Octree]
	{
		...
		for (const FChunkId ChunkId : ChunksToAdd)
		{
			...
			const TSharedPtr<FChunk> Chunk = Chunks_RequiresLock.Add_EnsureNew(ChunkId, MakeVoxelShared<FChunk>());
...
DEFINE_VOXEL_NODE_COMPUTE(FVoxelNode_MakeScreenSizeChunkSpawner, Spawner)
{
	...
	return VOXEL_ON_COMPLETE(WorldSize, ChunkSize, MaxLOD, EnableTransitions)
	{
		...
		Spawner->TerminalGraphInstance = Query.GetSharedTerminalGraphInstance();
