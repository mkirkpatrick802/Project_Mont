...
void FVoxelNode_UFunction::PreCompile()
{
	if (!ensure(Function))
	{
		return;
...
FVoxelComputeValue FVoxelNode_UFunction::CompileCompute(const FName ReturnPinName) const
{
	if (!ensure(Function))
	{
		return {};
	...
	return [this, ReturnPinId = ReturnPinData.PinId](const FVoxelQuery& InQuery) -> FVoxelFutureValue
	{
		...
		TVoxelInlineArray<FVoxelFutureValue, 8> InputValues;
		TVoxelInlineArray<TSharedRef<FVoxelFutureValueStateImpl>, 8> OutputStates;
		...
		.Execute([=]
		{
			TVoxelInlineArray<FVoxelRuntimePinValue*, 8> Values;
			TVoxelInlineArray<FVoxelRuntimePinValue, 8> InputBufferStorage;
			TVoxelInlineArray<FVoxelRuntimePinValue, 8> OutputStorage;
	...
	for (const FProperty& Property : GetFunctionProperties(Function))
	{
		...
		if (MetadataMap)
		{
			for (auto& It : *MetadataMap)
			{
				...
				if (Name == STATIC_FNAME("OptionalPin"))
				{
