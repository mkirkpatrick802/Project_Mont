...
DEFINE_VOXEL_NODE_COMPUTE(FVoxelNode_EqualBase_Object, Result)
{
	if (AreTemplatePinsBuffers())
	{
		...
		return VOXEL_ON_COMPLETE(A, B)
		{
			...
			const TSharedPtr<const FVoxelObjectPinType> ObjectPinType = FVoxelObjectPinType::StructToPinType().FindRef(A->GetInnerType().GetStruct());
			if (!ensure(ObjectPinType))
			{
				VOXEL_MESSAGE(Error, "{0}: Object pin must be Object type", this);
				return {};
	...
	else
	{
		...
		return VOXEL_ON_COMPLETE(A, B)
		{
			...
			const TSharedPtr<const FVoxelObjectPinType> ObjectPinType = FVoxelObjectPinType::StructToPinType().FindRef(A.GetType().GetStruct());
			if (!ensure(ObjectPinType))
			{
				VOXEL_MESSAGE(Error, "{0}: Object pin must be Object type", this);
				return {};
