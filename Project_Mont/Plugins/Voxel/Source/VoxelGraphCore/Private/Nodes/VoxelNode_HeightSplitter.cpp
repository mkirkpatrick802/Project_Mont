...
FVoxelComputeValue FVoxelNode_HeightSplitter::CompileCompute(const FName PinName) const
{
	// TODO QueryCache
	...
	return [this, Layer](const FVoxelQuery& Query) -> FVoxelFutureValue
	{
		...
		return VOXEL_ON_COMPLETE(HeightBuffer, HeightSplits, FalloffSplits, Layer)
		{
				...
				Layer < NumLayerPins)
			{
				...
				ForeachVoxelBufferChunk_Parallel(HeightBuffer.Num(), [&](const FVoxelBufferIterator& Iterator)
				{
