...
{
	const auto GetISPCNoise = [](const EVoxelAdvancedNoiseOctaveType Noise)
	{
		switch (Noise)
		{
...
#undef CASE
	...
	return VOXEL_ON_COMPLETE(Positions, Amplitudes, FeatureScales, Lacunarities, Gains, CellularJitters, NumOctaves, Seed, DefaultOctaveType, OctaveTypes, OctaveStrengths, GetISPCNoise)
	{
		...
		TVoxelInlineArray<ispc::FOctave, 16> OctavesInit;
		...
		{
			TVoxelInlineArray<ispc::FOctave, 16> Octaves = OctavesInit;
			for (int32 Index = 0; Index < SafeNumOctaves; Index++)
			{
...
DEFINE_VOXEL_NODE_COMPUTE(FVoxelNode_AdvancedNoise3D, Value)
{
	const auto GetISPCNoise = [](const EVoxelAdvancedNoiseOctaveType Noise)
	{
		switch (Noise)
