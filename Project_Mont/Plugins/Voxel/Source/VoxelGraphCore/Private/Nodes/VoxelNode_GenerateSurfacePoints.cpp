...
#include "Nodes/VoxelNode_GenerateSurfacePoints.h"
#include "VoxelRuntime.h"
#include "TransvoxelData.h"
#include "VoxelPositionQueryParameter.h"
#include "VoxelNode_GenerateSurfacePointsImpl.ispc.generated.h"
...
void FVoxelGenerateSurfacePointsBuilder::ComputeDistances()
{
	...
	{
		ProcessDistances(Surface.Get_CheckCompleted().GetDistance().GetStorage());
	}));
}
...
void FVoxelGenerateSurfacePointsBuilder::ComputeFinalDistances()
{
	...
	.Execute(MakeWeakPtrLambda(this, [=, ValueIndices = MoveTemp(ValueIndices)]
	{
		ProcessFinalDistances(
			NumQueries,
			ValueIndices,
			Surface.Get_CheckCompleted().GetDistance().GetStorage());
	}));
}
...
DEFINE_VOXEL_NODE_COMPUTE(FVoxelNode_GenerateSurfacePoints, Out)
{
	...
	return VOXEL_ON_COMPLETE(Bounds, CellSize, MaxResolution, DistanceChecksTolerance)
	{
		if (!Bounds.IsValid() ||
			Bounds == FVoxelBox())
		{
		...
		ensure(Bounds.Size().IsUniform());
		const double ChunkSize = Bounds.Size().X;

		if (FMath::RoundToFloat(ChunkSize / CellSize) != ChunkSize / CellSize)
		{
			VOXEL_MESSAGE(Warning, "{0}: ChunkSize should be a multiple of CellSize. ChunkSize={1}, CellSize={2}",
				this,
				ChunkSize,
				CellSize);
		}

		const FVoxelDummyFutureValue Dummy = FVoxelFutureValue::MakeDummy(STATIC_FNAME("GenerateSurfacePoints"));
