...
#include "Nodes/VoxelNode_GetPreviousOutput.h"
...
DEFINE_VOXEL_NODE_COMPUTE(FVoxelNode_GetPreviousOutput, Value)
{
	...
	return VOXEL_ON_COMPLETE(Name)
	{
		...
		const FVoxelChildQueryContextKey Key
		{
			...
			FVoxelGraphNodeRef(GetNodeRef().TerminalGraphRef, FVoxelNodeNames::GetPreviousOutputNodeId),
