...
VOXEL_CONSOLE_VARIABLE(
	VOXELGRAPHCORE_API, bool, GVoxelFoliageShowInstancesCollisions, true,
...
void FVoxelInstancedCollisionDataImpl::FlushInstanceBodiesToDelete()
{
	...
	for (const TSharedPtr<FBodyInstance>& Body : InstanceBodiesToDelete)
	{
		if (!Body)
		{
			// Body will be null when using SetPointVisibility
			continue;
		}
...
FBodyInstance* UVoxelInstancedCollisionComponent::GetBodyInstance(const FName BoneName, const bool bGetWelded, const int32 Index) const
{
	if (!Data)
	{
...

	{
		...
		if (DataImpl.InstanceBodies.IsValidIndex(Index))
		{
			return DataImpl.InstanceBodies[Index].Get();
		}
	}
...
void UVoxelInstancedCollisionComponent::SetData(const TSharedRef<FVoxelInstancedCollisionData>& NewData)
{
	...
	OverrideChunk = FVoxelPointManager::Get(GetWorld())->FindOrAddChunk(NewData->ChunkRef);
...
void UVoxelInstancedCollisionComponent::Update()
{
...

	{
		...
		for (const int32 Index : DataImpl.InstanceBodiesToUpdate)
		{
			...
			const FTransform Transform = FTransform(DataImpl.Transforms[Index]) * GetComponentTransform();
...
void UVoxelInstancedCollisionComponent::ResetOverlaps()
{
...
TSharedRef<FBodyInstance> UVoxelInstancedCollisionComponent::MakeBodyInstance(const int32 Index) const
{
	...
	FVoxelUtilities::CopyBodyInstance(*Body, BodyInstance);
