...
FVoxelPointCollisionSmallChunk::FVoxelPointCollisionSmallChunk(
	...
	, bGenerateOverlapEvents(bGenerateOverlapEvents)
	, bMultiBodyOverlap(bMultiBodyOverlap)
...
void FVoxelPointCollisionSmallChunk::UpdatePoints(const FMeshToPoints& MeshToPoints)
{
...

	{
		...
		for (const auto& It : MeshToComponent_RequiresLock)
		{
			const TVoxelMap<FVoxelPointId, FPointTransform>* PointToTransforms = MeshToPoints.Find(It.Key);
	...
	RunOnGameThread_Async(MakeWeakPtrLambda(this, [this]
	{
		...
		for (auto It = MeshToComponent_RequiresLock.CreateIterator(); It; ++It)
		{
			...
			if (!Component)
			{
				...
				Component->bMultiBodyOverlap = bMultiBodyOverlap;
