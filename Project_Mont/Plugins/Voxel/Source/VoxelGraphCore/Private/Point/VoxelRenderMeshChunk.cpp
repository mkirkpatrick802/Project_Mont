...
VOXEL_CONSOLE_VARIABLE(
	VOXELGRAPHCORE_API, int32, GVoxelMaxInstancesToAddAtOnce, 16,
...
FVoxelRenderMeshChunk::FVoxelRenderMeshChunk(
	...
	const TSharedRef<const FVoxelInstancedMeshComponentSettings>& MeshSettings)
	: NodeRef(NodeRef)
	, ChunkRef(ChunkRef)
	...
	, OverrideChunk(FVoxelPointManager::Get(RuntimeInfo->GetWorld())->FindOrAddChunk(ChunkRef))
{
}
...
void FVoxelRenderMeshChunk::UpdatePoints(const TSharedRef<const FVoxelPointSet>& NewPoints)
{
	...
	TVoxelMap<FVoxelPointId, int32> NewPointIdToIndex;
	{
	...
	TVoxelMap<FVoxelStaticMesh, TSharedPtr<FVoxelInstancedMeshData>> MeshToMeshData;
	{
	...
	TVoxelMap<FVoxelStaticMesh, TVoxelArray<int32>> MeshToInstancedInstancesToRemove;
	TVoxelMap<FVoxelStaticMesh, TVoxelArray<int32>> MeshToHierarchicalInstancesToRemove;
	{
	...
	for (const auto& It : MeshToMeshData)
	{
		...
		for (const FVoxelPointId PointId : MeshData.PointIds_Transient)
		{
			...
			if (InstancedInstancesToRemove.Num() > 0)
			{
				...
				Index = InstancedInstancesToRemove.Pop();
			}
			else if (Components.FreeInstancedIndices.Num() > 0)
			{
				Index = Components.FreeInstancedIndices.Pop();
			}
			else
			{
	...
	RunOnGameThread_Async(MakeWeakPtrLambda(this, [
		this,
		...
		MeshToHierarchicalInstancesToRemove = MoveTemp(MeshToHierarchicalInstancesToRemove)]
	{
		...
		for (const auto& It : MeshToMeshData)
		{
			...
			if (!Component->HasMeshData())
			{
				...
				MeshSettings->ApplyToComponent(*Component);
			}
			else
			{
...
void FVoxelRenderMeshChunk::UpdatePoints_Hierarchical_AssumeLocked(const TSharedRef<const FVoxelPointSet>& Points)
{
	...
	if (Points->Num() == 0)
	{
		RunOnGameThread_Async(MakeWeakPtrLambda(this, [this]
		{
	...
	FVoxelBufferDeduplicator::Deduplicate(Meshes, MeshIndices, MeshPalette);
	...
	RunOnGameThread_Async(MakeWeakPtrLambda(this, [this, HierarchicalMeshDatas]
	{
...
void FVoxelRenderMeshChunk::UpdatePointOverrides_AssumeLocked(const PointIdsType& PointIdsToUpdate)
{
	...
	for (const auto& It : MeshToComponents_RequiresLock)
	{
		...
		RunOnGameThread_Async(MakeWeakPtrLambda(this, [=, Components = It.Value]
		{
