...
VOXEL_CONSOLE_VARIABLE(
	VOXELGRAPHCORE_API, int32, GVoxelDetailTexturesDebug, 0,
	"voxel.detailtextures.debug",
	"0: Disabled\n"
	"1: Pixel density\n"
	"2: Ids\n"
	"3: Alphas\n"
...
DEFINE_VOXEL_MEMORY_STAT(STAT_VoxelMarchingCubeMeshMemory);
DEFINE_VOXEL_MEMORY_STAT(STAT_VoxelMarchingCubeMeshGpuMemory);
	...
	FVertexInputStreamArray& VertexStreams) const
{
	...
	ShaderBindings.Add(VoxelSize, float(VoxelVertexFactory.VoxelSize));
	...
	ShaderBindings.AddTexture(
		...
		TextureSampler,
		TStaticSamplerState<SF_Bilinear>::GetRHI(),
		VoxelVertexFactory.Material_TextureA ? VoxelVertexFactory.Material_TextureA : GBlackTexture->TextureRHI);
	ShaderBindings.AddTexture(
		Material_TextureB,
		TextureSampler,
		TStaticSamplerState<SF_Bilinear>::GetRHI(),
		VoxelVertexFactory.Material_TextureB ? VoxelVertexFactory.Material_TextureB : GBlackTexture->TextureRHI);

	ShaderBindings.Add(Material_Texture_Size, VoxelVertexFactory.Material_Texture_Size);
	ShaderBindings.Add(Material_Texture_InvSize, VoxelVertexFactory.Material_Texture_InvSize);
...
void FVoxelMarchingCubeMesh::SetTransitionMask_GameThread(uint8 NewTransitionMask)
{
	...
	if (UVoxelMarchingCubeMeshComponent* Component = WeakComponent.Get())
	{
...
TSharedRef<FVoxelMaterialRef> FVoxelMarchingCubeMesh::GetMaterial() const
{
	if (GVoxelDetailTexturesDebug != 0)
	{
		static const TMap<int32, FString> DebugToName =
		{
			{ 1, "M_PixelDensity" },
			{ 2, "M_Ids" },
			{ 3, "M_Alphas" },
		...
		const FString* NamePtr = DebugToName.Find(GVoxelDetailTexturesDebug);
		if (!NamePtr)
		{
			return FVoxelMaterialRef::Default();
		}

		static TMap<int32, TSharedPtr<FVoxelMaterialRef>> DebugToMaterialRef;

		TSharedPtr<FVoxelMaterialRef>& MaterialRef = DebugToMaterialRef.FindOrAdd(GVoxelDetailTexturesDebug);
		if (!MaterialRef)
		{
			const FString Path = "/Voxel/Debug/VoxelDetailTextureDebug/" + *NamePtr + "." + *NamePtr;
			MaterialRef = FVoxelMaterialRef::Make(LoadObject<UMaterialInterface>(nullptr, *Path));
		}
...
void FVoxelMarchingCubeMesh::Initialize_GameThread(UVoxelMarchingCubeMeshComponent& InComponent)
{
	VOXEL_FUNCTION_COUNTER();
	check(IsInGameThread());
	check(ComputedMaterial);

	if (!ensure(WeakComponent.IsExplicitlyNull()))
	{
		return;
	}
	...
	if (const TSharedPtr<FVoxelDynamicMaterialParameter> MaterialParameter = Material->FindDynamicParameter(STATIC_FNAME("Material")))
	{
		const TSharedPtr<FVoxelDetailTextureDynamicMaterialParameter> DetailTexture = Cast<FVoxelDetailTextureDynamicMaterialParameter>(MaterialParameter);
		if (ensure(DetailTexture))
		{
				...
				&DetailTexture = *DetailTexture]
			{
				...
				VOXEL_ENQUEUE_RENDER_COMMAND(UpdateTexture)([VertexFactory = VertexFactory, ResourceA, ResourceB](FRHICommandListImmediate& RHICmdList)
				{
					...
					VertexFactory->Material_TextureA = ResourceA->GetTexture2DRHI();
					VertexFactory->Material_TextureB = ResourceB->GetTexture2DRHI();
					VertexFactory->Material_Texture_Size = ResourceA->GetSizeX();
					VertexFactory->Material_Texture_InvSize = 1. / double(ResourceA->GetSizeX());
				});
			}));
	...
	if (const float* Material_TextureSize = Material->FindScalarParameter(STATIC_FNAME("Material_TextureSize")))
	{
		VertexFactory->Material_TextureSize = *Material_TextureSize;
	}

	if (const float* Material_TextureIndex = Material->FindScalarParameter(STATIC_FNAME("Material_TextureIndex")))
	{
		VertexFactory->Material_TextureIndex = FMath::RoundToInt(*Material_TextureIndex);
	}
...
void FVoxelMarchingCubeMesh::InitializeIfNeeded_RenderThread(FRHICommandListBase& RHICmdList, ERHIFeatureLevel::Type FeatureLevel)
{
	VOXEL_FUNCTION_COUNTER();
...
TSharedRef<FVoxelMarchingCubeMesh> FVoxelMarchingCubeMesh::New()
{
	const TSharedRef<FVoxelMarchingCubeMesh> Result = TSharedPtr<FVoxelMarchingCubeMesh>(new (GVoxelMemory) FVoxelMarchingCubeMesh(), [](FVoxelMarchingCubeMesh* Mesh)
	{
		if (!Mesh)
		{
			return;
	...
	SharedPointerInternals::EnableSharedFromThis(&Result, &Result.Get(), &Result.Get());
