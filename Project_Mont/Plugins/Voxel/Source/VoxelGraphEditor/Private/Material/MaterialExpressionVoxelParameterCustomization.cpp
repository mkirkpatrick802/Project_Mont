...
public:
	virtual void CustomizeDetails(IDetailLayoutBuilder& DetailLayout) override
	{
		...
		[
			...
			[
				...
				.OptionText(MakeLambdaDelegate([this](const FGuid Guid) -> FString
				{
					...
					const FVoxelMaterialDefinitionParameter* Parameter = MaterialDefinition->GuidToMaterialParameter.Find(Guid);
					if (!Parameter)
					{
						return "Invalid";
					}
	...
	void UpdateMaterialDefinitionData()
	{
		...
		for (const auto& It : WeakMaterialDefinition->GuidToMaterialParameter)
		{
			if (!ValidTypes.Contains(It.Value.Type))
			{
				continue;
			}

			AvailableParameters.Add(It.Key);
		}
