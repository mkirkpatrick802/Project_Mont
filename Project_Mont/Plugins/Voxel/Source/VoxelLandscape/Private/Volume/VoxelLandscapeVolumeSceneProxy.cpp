...
VOXEL_CONSOLE_VARIABLE(
	VOXELLANDSCAPE_API, bool, GVoxelLandscapeShowVolumeSections, false,
	"voxel.landscape.ShowVolumeSections",
	"");

FVoxelLandscapeVolumeSceneProxy::FVoxelLandscapeVolumeSceneProxy(const UVoxelLandscapeVolumeComponent& Component)
...
void FVoxelLandscapeVolumeSceneProxy::DrawStaticElements(FStaticPrimitiveDrawInterface* PDI)
{
...
void FVoxelLandscapeVolumeSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, const uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	VOXEL_FUNCTION_COUNTER();
	...
	if (GVoxelLandscapeShowVolumeSections)
	{
...
FPrimitiveViewRelevance FVoxelLandscapeVolumeSceneProxy::GetViewRelevance(const FSceneView* View) const
{
...
bool FVoxelLandscapeVolumeSceneProxy::CanBeOccluded() const
{
	return !MaterialRelevance.bDisableDepthTest;
}

uint32 FVoxelLandscapeVolumeSceneProxy::GetMemoryFootprint() const
{
	return sizeof(*this) + GetAllocatedSize();
}

SIZE_T FVoxelLandscapeVolumeSceneProxy::GetTypeHash() const
{
...
void FVoxelLandscapeVolumeSceneProxy::GetDistanceFieldAtlasData(const FDistanceFieldVolumeData*& OutDistanceFieldData, float& SelfShadowBias) const
{
	OutDistanceFieldData = Mesh->DistanceFieldVolumeData.Get();
	SelfShadowBias = Mesh->SelfShadowBias;
}

void FVoxelLandscapeVolumeSceneProxy::GetDistanceFieldInstanceData(TArray<FRenderTransform>& ObjectLocalToWorldTransforms) const
{
	if (FVoxelLandscapeVolumeSceneProxy::HasDistanceFieldRepresentation())
	{
		ObjectLocalToWorldTransforms.Add(FRenderTransform::Identity);
	}
}

const FCardRepresentationData* FVoxelLandscapeVolumeSceneProxy::GetMeshCardRepresentation() const
{
	return Mesh->CardRepresentationData.Get();
}

bool FVoxelLandscapeVolumeSceneProxy::HasDistanceFieldRepresentation() const
{
...
bool FVoxelLandscapeVolumeSceneProxy::HasDynamicIndirectShadowCasterRepresentation() const
{
	return bCastsDynamicIndirectShadow && FVoxelLandscapeVolumeSceneProxy::HasDistanceFieldRepresentation();
}
...
bool FVoxelLandscapeVolumeSceneProxy::ShouldUseStaticPath(const FSceneViewFamily& ViewFamily)
{
	return
		!IsRichView(ViewFamily) &&
		!GVoxelLandscapeShowVolumeSections;
}

bool FVoxelLandscapeVolumeSceneProxy::DrawMesh(
	FMeshBatch& MeshBatch,
