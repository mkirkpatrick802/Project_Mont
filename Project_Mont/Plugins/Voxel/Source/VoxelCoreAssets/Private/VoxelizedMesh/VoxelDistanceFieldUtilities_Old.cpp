	...
	const FVoxelIntBox* InBounds)
{
	...
	FScopedSlowTask SlowTask(PowerOfTwo, INVTEXT("Jump Flood"));
...
void FVoxelDistanceFieldUtilities::GetDistancesFromSurfacePositions(
	const FIntVector& Size,
	const TVoxelArrayView<const FVector3f> SurfacePositions,
	const TVoxelArrayView<float> InOutDistances,
	const FVoxelIntBox* InBounds)
{
...
void FVoxelDistanceFieldUtilities::GetSurfacePositionsFromDensities(
	const FIntVector& Size,
	const TVoxelArrayView<const float> Densities,
	const TVoxelArrayView<float> OutDistances,
	const TVoxelArrayView<FVector3f> OutSurfacePositions,
	const bool bParallel,
	...
	if (bParallel)
	{
		...
		ParallelFor(NumThreads, [&](const int32 ThreadIndex)
		{
			...
			for (int32 Z = StartIndex; Z < EndIndex; Z++)
			{
				if (OutBounds)
				{
					Body(std::true_type(), Z);
				}
				else
				{
					Body(std::false_type(), Z);
				}
			}
		});
	}
...
void FVoxelDistanceFieldUtilities::JumpFloodStep_CPU(
	...
	const TVoxelArrayView<const FVector3f> InData,
	const TVoxelArrayView<FVector3f> OutData,
	const int32 Step,
	const bool bParallel)
{
	VOXEL_FUNCTION_COUNTER();
...
#undef CheckNeighbor_DZ
	...
	const auto DispatchDoWork = [&](const int32 Z)
	{
		switch (Step)
		{
